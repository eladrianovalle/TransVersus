...
//        [Range(0, 1)] [SerializeField] private float m_CrouchSpeed = .36f;  // Amount of maxSpeed applied to crouching movement. 1 = 100%
		...
		private float airControlTimeLimit = .5f; 		// max amount of time that can have air control
		private float canWallJumpTimeLimit = .25f; 		// max amount of time that can try a wall jump
...
//            m_Anim = GetComponent<Animator>();
		...
		void Update()
		{
			...
			{
				airControlTimer -= Time.unscaledDeltaTime;
			} 
			else 
			{
				m_AirControl = false;
        ...
        private void FixedUpdate()
        {
			...
			Collider2D[] wallCheckColliders = Physics2D.OverlapCircleAll(m_WallCheck.position, k_WallRadius, m_WhatIsWall);
			for (int i = 0; i < wallCheckColliders.Length; i++)
			{
				if (wallCheckColliders [i].gameObject != gameObject) 
				{
					m_CanWallJump = true;
					wallJumpTimer = canWallJumpTimeLimit;
				}
			}
